...
export default function BenefitCard({ row, now }: { row: BenefitListRow; now?: Date }) {
  ...
  return (
    <article className="relative flex h-full flex-col rounded-xl border border-gray-200 bg-white p-4 transition hover:border-indigo-300 hover:shadow-sm focus-within:border-indigo-400">
      ...
      <h3 className="text-base font-semibold leading-snug">
        ...
        <Link href={`/benefit/${row.slug}`} className="after:absolute after:inset-0 after:rounded-xl hover:text-indigo-700">
          {row.title}
        </Link>
      </h3>
