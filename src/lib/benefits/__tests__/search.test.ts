...
describe('parseSearchParams', () => {
  ...
  it('소득 구간도 받는다', () => {
    ...
    expect(parseSearchParams(new URLSearchParams('income=200%2B')).incomeBand).toBe('200+')
  })
  it('허용되지 않은 값은 버린다', () => {
    const p = parseSearchParams(new URLSearchParams('age=99s&situations=x,job_seeker&region=mars&limit=999&offset=-3&income=1-2'))
    expect(p.ageBand).toBeNull()
    expect(p.situations).toEqual(['job_seeker'])
    expect(p.region).toBeNull()
    expect(p.incomeBand).toBeNull()
...
describe('matchesConditions', () => {
  const cond = { age_min: 19, age_max: 34, gender: 'any' as const, life_stages: ['pregnancy'], household_types: [], occupations: [], region_codes: ['seoul'], income_bands: [] }
  it('나이 범위 겹치면 통과', () => {
    expect(matchesConditions(cond, { ageRange: [30, 39], situations: [], region: null, incomeBands: null })).toBe(true)
    expect(matchesConditions(cond, { ageRange: [40, 49], situations: [], region: null, incomeBands: null })).toBe(false)
  })
  it('한쪽만 설정된 나이 조건도 거른다', () => {
    const onlyMin = { ...cond, age_min: 65, age_max: null }
    expect(matchesConditions(onlyMin, { ageRange: [20, 29], situations: [], region: null, incomeBands: null })).toBe(false)
    expect(matchesConditions(onlyMin, { ageRange: [50, 120], situations: [], region: null, incomeBands: null })).toBe(true)
    const onlyMax = { ...cond, age_min: null, age_max: 18 }
    expect(matchesConditions(onlyMax, { ageRange: [20, 29], situations: [], region: null, incomeBands: null })).toBe(false)
    expect(matchesConditions(onlyMax, { ageRange: [10, 19], situations: [], region: null, incomeBands: null })).toBe(true)
  })
  it('나이 조건이 없는 항목은 나이로 거르지 않는다', () => {
    expect(matchesConditions({ ...cond, age_min: null, age_max: null }, { ageRange: [40, 49], situations: [], region: null, incomeBands: null })).toBe(true)
  })
  ...
  */
  it('상한이 10대 밑이면 신청자 나이로 읽지 않는다 — 아동수당이 모든 밴드에서 사라졌었다', () => {
    ...
    for (const band of [[10, 19], [20, 29], [30, 39], [40, 49], [50, 120]] as [number, number][])
      expect(matchesConditions(childAge, { ageRange: band, situations: [], region: null, incomeBands: null })).toBe(true)
    // 나이로 가점도 하지 않는다. 신청자 나이가 아니니 맞물렸다고 볼 수 없다.
    expect(matchScore(childAge, { ageRange: [30, 39], situations: [], region: null, incomeBands: null })).toBe(0)
  })

  it('경계: 상한 9는 신청자 나이가 아니고, 10은 신청자 나이다', () => {
    const nine = { ...cond, age_min: 0, age_max: 9 }
    const ten = { ...cond, age_min: 0, age_max: 10 }
    expect(matchesConditions(nine, { ageRange: [30, 39], situations: [], region: null, incomeBands: null })).toBe(true)
    expect(matchesConditions(ten, { ageRange: [30, 39], situations: [], region: null, incomeBands: null })).toBe(false)
    expect(matchesConditions(ten, { ageRange: [10, 19], situations: [], region: null, incomeBands: null })).toBe(true)
  })

  it('청소년 대상 사업은 그대로 걸러진다 — 상한 18은 10대와만 겹친다', () => {
    const teen = { ...cond, age_min: 13, age_max: 18 }
    expect(matchesConditions(teen, { ageRange: [10, 19], situations: [], region: null, incomeBands: null })).toBe(true)
    expect(matchesConditions(teen, { ageRange: [30, 39], situations: [], region: null, incomeBands: null })).toBe(false)
  })
  it('상황은 하나라도 일치하면 통과, 상황 조건이 없는 항목은 통과', () => {
    expect(matchesConditions(cond, { ageRange: null, situations: ['pregnancy'], region: null, incomeBands: null })).toBe(true)
    expect(matchesConditions(cond, { ageRange: null, situations: ['job_seeker'], region: null, incomeBands: null })).toBe(false)
    expect(matchesConditions(cond, { ageRange: null, situations: ['job_seeker', 'pregnancy'], region: null, incomeBands: null })).toBe(true)
    expect(matchesConditions({ ...cond, life_stages: [] }, { ageRange: null, situations: ['job_seeker'], region: null, incomeBands: null })).toBe(true)
  })
  it('지역은 일치 또는 전국(빈 배열)', () => {
    expect(matchesConditions(cond, { ageRange: null, situations: [], region: 'seoul', incomeBands: null })).toBe(true)
    expect(matchesConditions(cond, { ageRange: null, situations: [], region: 'busan', incomeBands: null })).toBe(false)
    expect(matchesConditions({ ...cond, region_codes: [], income_bands: [] }, { ageRange: null, situations: [], region: 'busan', incomeBands: null })).toBe(true)
  })
})

describe('matchScore', () => {
  const base = { age_min: null, age_max: null, gender: 'any' as const, life_stages: [], household_types: [], occupations: [], region_codes: [], income_bands: [] }
  const q = { ageRange: [20, 29] as [number, number], situations: ['job_seeker'], region: 'seoul', incomeBands: null }
  it('상황 일치 +2, 지역 일치 +1, 나이 조건 일치 +1', () => {
    ...
    expect(matchScore({ ...base, region_codes: ['seoul'], income_bands: [] }, q)).toBe(1)
  ...
  it('사용자가 상황을 고르지 않았으면 상황 점수를 주지 않는다', () => {
    expect(matchScore({ ...base, occupations: ['job_seeker'] }, { ageRange: null, situations: [], region: null, incomeBands: null })).toBe(0)
  })
  it('전국(region_codes 빈 배열) 항목은 지역 점수를 못 받는다', () => {
    expect(matchScore(base, { ageRange: null, situations: [], region: 'seoul', incomeBands: null })).toBe(0)
  })
})
...
describe('대상이 아이인 행', () => {
  ...
  const hasChild = { ageRange: null, situations: ['has_child'], region: null, incomeBands: null }
  const jobSeeker = { ageRange: null, situations: ['job_seeker'], region: null, incomeBands: null }
  ...
  it('임신 중만 골랐을 때는 나이만으로 걸리지 않는다', () => {
    const pregnancy = { ageRange: null, situations: ['pregnancy'], region: null, incomeBands: null }
  ...
  it('원천이 준 생애주기가 있으면 그것으로도 걸린다', () => {
    ...
    expect(matchScore(withCode, { ageRange: null, situations: ['pregnancy'], region: null, incomeBands: null })).toBe(2)
  })
