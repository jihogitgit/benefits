...
- 검색 랭킹 → **해결.** Plan 2 Task 13에서 `matchScore`(상황 2·지역 1·나이 1)를 `rankBenefits`의 1차 정렬 키로 넣었다. 조건이 아예 등록되지 않은 전 국민 대상 항목은 점수 0으로 뒤로 간다. 상황 점수는 조건의 *존재*가 아니라 *실제 일치*로 준다(`matchesConditions`와 판정 로직 공유).
- 세그먼트 오탐 잔여: "아동" 키워드가 장애아동·아동복지시설 등을 출산·육아로 잡는다. 색인은 검수된 항목만 하므로 Phase 1 영향은 없고, Plan 3 검수 화면에서 세그먼트 수정 기능으로 보정한다.
- 브랜드명 → **확정: `내몫`** (2026-09-15). 지원금은 시혜가 아니라 요건을 갖추면 받는 권리라는 프레임을
  담았다. 정부 서비스 사칭으로 읽히지 않도록 `정부`·`국민`·`나라` 접두어와 `24` 접미어는 배제했다
  (실제 정부 서비스인 보조금24·정부24와 혼동을 피한다). 코드에서는 `SITE_NAME` 환경변수이고
  미설정 시 폴백도 `내몫`이다. **도메인은 아직 미확정.**
- 브랜드 색 → **확정: teal 계열** (2026-09-15). 보조금24·정부24·복지로가 모두 파랑이라 기존 indigo로는
  구분되지 않았다. `src/app/globals.css`의 `--color-brand-*` 8줄이 단일 출처이고 컴포넌트는 `brand-*`
  유틸리티만 쓴다. 주의: teal-600(#0D9488)은 흰 배경 대비 3.74:1로 WCAG AA 미달이라 본문·버튼에
  쓸 수 없다. 그래서 `brand-600`에 teal-700(5.47:1)을 물렸다. 사용 중인 모든 전경/배경 쌍이 AA를 통과한다.
- 배지 위계 재정의: 빨강(14일 이내) > 브랜드색(그 외 D-day) > 진회색(상시) > 연회색(공고 확인).
  `상시`를 emerald에서 회색으로 내린 이유는 브랜드 청록과 한 화면에서 구분되지 않기 때문이다.
- 지역 안내문 17개의 작성 → 검수 화면에서 AI 초안 + 검수로 동일 처리.

### Plan 2 실행 중 발견 (2026-09-10, Task 6 검토)

- **광주 지역이 실질적으로 비어 있다.** 소관기관명 실데이터가 `전남광주통합특별시`(및 `전남광주통합특별시 서구` 등)로
  들어와 있어 `extractRegion`이 전부 `jeonnam`으로 잡는다. 결과: `jeonnam` 1,053건, `gwangju` 1건.
  「광주 포함」 소관기관명은 1,113건이며 모두 통합 명칭이다.
  `data/regions.ts`는 여전히 광주와 전남을 별도 시도로 두므로, Task 9의 지역 칩과 Task 12의 사이트맵이
  1건짜리 광주 페이지를 만들게 된다. 이는 색인 정책(검수된·내용 있는 페이지만 색인)과 정면으로 충돌한다.
  **Task 9 착수 전에 결정이 필요하다.** 후보:
  (a) `gwangju`를 목록에서 빼고 `jeonnam`의 표시명을 통합 명칭에 맞춰 바꾼다(URL 1개 감소),
  (b) 두 슬러그를 유지하되 `gwangju`는 `jeonnam`으로 리다이렉트한다,
  (c) 지역 목록을 건수 임계값으로 자동 필터한다(구조 변경 없음, 표시명 문제는 남는다).
  어느 쪽이든 지역 칩·사이트맵은 건수 0~1인 지역을 링크하지 않아야 한다.
- 참고: `benefits.region_code`는 이름과 달리 행정표준코드가 아니라 `data/regions.ts`의 **slug**(`seoul`, `gyeonggi`)를
  담는다. `regions` 테이블의 `code`(`11`, `26`)와 다르다. 카드·칩에서 이름을 찾을 때 slug 기준으로 조회해야 한다.

### Plan 2 완료 (2026-09-15, Task 10~14)

구현 완료: 지원금 상세(`/benefit/{slug}`), 마감 캘린더(`/deadline`), 가이드(`/guide/{slug}`),
분할 사이트맵·robots·OG 이미지, 검색 정렬 개선. 게이트: 162 테스트 / tsc 0 / eslint 0 / 빌드 72페이지.

Task 10~14 실행 중 플랜 원문에서 발견해 고친 결함:

- **사이트맵 id가 문자열로 넘어온다.** Next는 `generateSitemaps`의 id를 경로 조각 그대로(`'0'`) 넘기므로
  `id === 0` 분기가 거짓이 되어 정적 항목과 지역 허브가 사이트맵에서 통째로 빠졌다(실측 0건).
  `Number(id)`로 수치화해 9건이 되었다.
- **`/sitemap.xml` 인덱스는 Next가 만들지 않는다.** `generateSitemaps`는 `/sitemap/{id}.xml`만 낸다.
  `robots.txt`가 가리키던 `/sitemap.xml`은 404였다. 분할 파일 목록(`sitemapPaths()`)을 단일 출처로 삼아
  robots가 전부 나열하도록 바꿨다.
- **사이트맵의 지역 건수 집계가 Supabase 1000행 상한에 잘렸다.** Task 6에서 `countByRegion`에 대해
  고쳤던 것과 같은 결함이 사이트맵 코드에 다시 들어 있었다. 이미 페이징·캐시된 `countByRegion`을
  재사용하도록 바꿨고, 세그먼트별 상세 질의도 `range()` 순회로 감쌌다(Plan 3에서 1000건을 넘길 수 있다).
- **OG 이미지의 `runtime = 'edge'`가 정적 생성을 껐다.** 내용이 고정인 이미지가 요청마다 렌더되고 있었다.
  런타임 지정을 빼서 빌드 시 1회 생성으로 되돌렸다.

실측 결과:

- `/sitemap/0.xml` 9건(홈·분야 3·마감·필수 4). 지역 허브 0건은 정상 — `regions.description_md`가 전부
  null이라 `regionHubIndexable`이 false다. `/sitemap/1~3.xml` 0건도 정상(`benefit_articles` 0행).
- 상세 페이지는 전 건 `noindex`, JSON-LD는 `GovernmentService`·`BreadcrumbList` 2개. 해설이 없으므로
  원문 안내 경로로 렌더된다.
- `/deadline` 45일 기준 19건(이번 주 2 · 다음 주 1 · 이달 안 6 · 그 이후 10).
- 검색 "20대·구직·서울" 상위 20건이 전부 score 4. 조건 미등록 항목은 상단에서 사라졌다.
  단, "(기초생활수급자)인플루엔자 백신 및 접종 지원"은 원천 데이터가 실제로 구직자 조건을 달고 있어
  18위에 남는다. 이는 정렬이 아니라 원천 태깅 문제이고, Plan 3 검수에서 다룬다.

코드 리뷰(별도 레인)에서 나와 반영한 것:

- **나이 부분 겹침을 '충족'으로 표시하던 문제.** 진단은 나이를 10년 단위로만 받는데 조건 구간과
  1살만 겹쳐도 체크 + "바로 신청해 보세요"가 나갔다. `'50s+'`가 [50,120]이라 `age_min >= 60`인
  조건 858건에 대해 50세 사용자가 "만 65세 이상"을 충족으로 보게 된다. `CheckState`에 `partial`을
  추가해 **완전히 포함될 때만 pass**로 바꾸고, partial은 체크를 비우고 "나이 확인 필요"를 붙이며
  "바로 신청해 보세요"는 partial이 하나도 없을 때만 낸다.
- **`Markdown`이 제목 바로 아래 본문을 삭제하던 문제.** 빈 줄 없이 제목에 이어 쓴 본문이 통째로
  사라졌다. 콘텐츠가 0행인 지금은 드러나지 않지만 Plan 3에서 전 상세·가이드에 적용됐을 문제다.
- **`age_min`·`age_max`가 각각 독립 nullable인데 둘 다 있을 때만 처리하던 문제.** 체크리스트 생성과
  검색 필터 양쪽에 있었다. 실데이터에는 한쪽만 있는 행이 2건이다.

2차 리뷰에서 추가로 반영한 것:

- **마감된 지원금인데 데스크톱 레일에는 신청 CTA가 남았다.** 모바일 하단 바만 `closed`로 숨기고
  `StickyRail`에는 전달하지 않아, 같은 화면에서 "마감되었습니다" 안내와 "공식 사이트에서 신청" 버튼이
  동시에 보였다. 뷰포트에 따라 안내가 모순됐다.
- **`matchScore`의 나이 가점이 '일치'가 아니라 '존재'만 봤다.** export된 함수라 사전 필터 통과를
  전제할 수 없는데 JSDoc은 '일치'라고 선언하고 있었다. 실제 겹침 검사로 바꾸고 `AGE_MAX = 120`을
  상수로 묶어 `ageBandToRange`·`matchesConditions`·`checklist`가 같은 경계값을 쓰게 했다.
- **`Checklist`의 effect가 사용자의 수동 체크를 되돌릴 수 있었다.** deps가 서버 컴포넌트에서 내려온
  객체 참조라 RSC 페이로드 갱신마다 재실행됐다. 항목 키 문자열을 deps로 써서 항목 구성이 실제로
  바뀔 때만 다시 프리필한다.
- **진단을 이미 채운 사용자에게 "홈에서 조건을 고르면"이라 안내했다.** 성별·소득 조건만 있는
  지원금은 진단에 해당 입력이 없어 전부 unknown이 되는데, 그때 방금 한 일을 다시 하라는 문구가
  나갔다. `isEmpty(d)`로 갈라 "자동 판정할 수 있는 항목이 없습니다"로 바꿨다.
- **사이트맵의 `lastmod`가 항상 현재 시각이었다.** `revalidate=3600`과 맞물려 `/privacy`까지 매시간
  수정됐다고 신고했다. 정적·허브 항목의 `lastmod`를 없앴다(선택 항목이다). 상세 항목은
  `source_updated_at`·`reviewed_at`의 최댓값으로 정확히 계산한 값을 그대로 유지한다.
- **`SEGMENT_BY_SLUG`가 미지 값에 방어가 없었다.** `benefits.segments`는 CHECK 제약 없는 `text[]`라
  코드보다 앞선 동기화나 수동 수정으로 미지 값이 들어오면 상세 페이지가 500이 된다. `SEGMENT_BY_PATH`와
  같은 규율(undefined를 타입에 남김)로 맞추고 `other`로 폴백한다.

3차 리뷰에서 추가로 반영한 것:

- **성별·소득 항목이 영원히 빈 체크박스로 남았다.** `Diagnosis`에 해당 입력이 없어 항상 unknown인데
  아무 표시가 없어 "나는 자격이 없다"로 읽혔다. 소득 조건 2,173건 · 성별 조건 825건(전체 10,947)으로
  4분의 1가량이 해당한다. `isUnjudgeable()`로 구분해 "· 직접 확인"을 붙인다.
- **전 연령 조건(0~제한없음)이 "만 0세 이상"이라는 잡음 항목이 됐다.** 제한이 없다는 뜻이므로
  체크 항목으로 만들지 않는다.
- `/deadline`의 `limit(200)` 재검토 트리거를 코드 주석에 남겼다 — `deadline_type='period' AND
  status='open'`이 200을 넘는 시점.

리뷰에서 지적됐으나 실데이터·실측 확인 후 보류한 것:

- `/deadline`의 `limit(200)` 절단 우려 → 45일 내 마감이 19건이고 기간형 open 전체가 160건이라
  현재 구조로는 절단이 불가능하다. 건수가 늘면 총계 표시와 분리가 필요하다.
- 검수된 `related_ids`가 조회만 되고 쓰이지 않는다 → 큐레이션 UI가 나오는 Plan 3 범위다. 다만
  검수 공수가 화면에 도달할 경로가 없는 구조이므로 **Plan 3 시작 전에 쓸지/뺄지 결론이 필요하다.**
- `decodeURIComponent` 이중 디코딩으로 500이 난다는 지적 → 실측 결과 `/benefit/100%`·`/guide/a%zz`
  모두 **400**을 반환한다(500 아님). 잘못된 URI에 400은 올바른 응답이라 방어 코드를 넣지 않았다.
- 세그먼트 사이트맵이 색인 불가 행까지 가져와 메모리에서 버린다 → 질의로 내리면 색인 판정이
  `index-policy.ts` 밖으로 새어나간다. `revalidate=3600`이라 비용도 크지 않아 현 구조를 유지한다.
- 성별·소득 조건은 진단에 해당 입력이 없어 영원히 unknown이다 → 요약 문구로 상황을 정확히 알리는
  데까지만 처리했다. 진단에 소득 구간을 추가하는 것은 별도 결정이다.

### SEO 1단계 완료 (2026-09-15)

지역 안내문 17개를 `data/regions.ts`에 넣고 `npm run seed:static`으로 반영했다. 안내문은 DB가 아니라
코드에 둔다 — git diff로 리뷰되고 세그먼트 설명과 관리 방식이 같아진다.

**색인 URL 9개 → 55개** (정적 9 + 지역 허브 46). 지역별 주요 소관기관 분포와 세그먼트 구성을 실제로
조회해 안내문을 각기 다르게 썼다. 지역명만 갈아끼운 템플릿은 얇은 콘텐츠라 색인을 열어도 얻는 게 없다.

남은 SEO 작업(효과 순):

1. **상세 콘텐츠 검수** — 검색 수요의 대부분이 개별 사업명(`근로장려금 신청기간`, `청년도약계좌 조건`)에
   있는데 상세 10,947건이 전부 noindex다. 전량은 불가능하니 검색량 상위 100~200건부터 연다.
2. **가이드 페이지** — `guides` 0행. "지원금 신청방법", "중위소득 계산" 같은 정보성 검색어 자리가 비어 있다.
3. **상세 제목에 `신청기간` 추가**, **허브 FAQ 블록 + FAQPage 스키마**, **홈에 인기 지원금 내부 링크**.

배포 운영 주의:

- `/api/benefits/search`는 Redis `getOrSet`(TTL 600초)을 쓰고 캐시 키에 코드 버전이 없다. 검색 필터·
  정렬을 바꾼 배포 직후 최대 10분간 옛 결과가 서빙되므로, QA 전에 기다리거나 `benefits:search:*`를 비운다.

관찰(이번 범위 밖, Plan 3 검수에서 볼 것):

- `benefit_conditions`에 `age_min=65, age_max=10`처럼 하한>상한인 행이 있다. 현재 판정에서는 항상
  fail로 떨어져 안전하게 degrade되지만 원천·정규화 확인이 필요하다.
- "성북구 평생학습센터 수강료 감면(경로우대자)"의 조건에 농업인·어업인·축산업인·임업인이 함께 붙어
  체크리스트가 10줄이 된다. 그룹 전체 Y = '제한 없음' 규약이 일부 코드군에 덜 적용된 것으로 보인다.

여전히 사람이 해야 하는 일(코드로 불가):

1. Supabase 마이그레이션 002(`sync_runs` anon read 정책) 미적용 — 홈 하단 "최종 동기화"가 "확인 중"으로 남는다.
2. ~~광주 taxonomy 결정~~ → **해소됨 (2026-09-15).** 색인 판정을 '지역 전용' 건수로 바꾸면서
   광주 3개 페이지가 자동으로 제외됐다(지역 전용 1/0/0). 별도 슬러그 삭제나 리다이렉트 없이
   후보 (c) 건수 기반 필터로 확정한다. 광주 안내문에는 「전남광주통합특별시」 사정과 전남 페이지로
   가라는 안내를 적어 두었다.
3. 광고·분석 환경변수. `NEXT_PUBLIC_CONTACT_EMAIL`은 애드센스 심사에 필요하므로 배포 전 필수.
