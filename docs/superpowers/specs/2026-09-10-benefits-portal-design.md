...
## 12. 열린 질문 (Phase 1 진행 중 확정)

- 보조금24 지원조건 코드표의 실제 값과 소득 구간 표현 방식 → 키 발급 후 Swagger·샘플로 확정, `codemap.ts`에 반영.
- 브랜드명·도메인 → Phase 4 전까지 결정. 코드에서는 `SITE_NAME`, `SITE_URL` 환경변수.
- 지역 안내문 17개의 작성 주체 → 검수 화면에서 AI 초안 + 검수로 동일 처리.
